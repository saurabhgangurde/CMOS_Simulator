...
T = 300;            		% temperature (K)
Ns =-2.5E15;    			% Substrate doping concentration (cm^-3)
				% (-/+ for n/p-type);
...
Vstart=(-0.5)*np*1;       	% Silicon Voltage: start (accumulation)
Vend=np*0.5;         		% Silicon Voltage: end   (inversion)
...
x = zeros(Nvstep,1);
for i=1:Nvstep;
    x(i)=(i-1)*(fregion+aregion)/Nvstep;
end
plot(x,Vs0)
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%
